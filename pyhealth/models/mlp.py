...
class MLP(BaseModel):
    ...
    Examples:
        ...
        >>> logits
        tensor([[0.2736], [0.1411]], grad_fn=<AddmmBackward0>)
        >>>
    ...
    def forward(self, **kwargs) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Returns:
            logits: a tensor representing the predicted probabilities.
            patient_emb: final layer tensor representing the patient embedding.
        """
        patient_emb = []
        for feature_key in self.feature_keys:
            input_info = self.dataset.input_info[feature_key]
...
if __name__ == "__main__":
    ...
    # try the model
    logits, patient_emb = model.forward(**data_batch)
    ret = model.fit(**data_batch)
    print(logits, ret)
